...
const staticWorkerCount = Math.max(1, Number.parseInt(process.env.STATIC_FONT_THREADS ?? "1", 10));
...
`
		...
		logger.debug("新字數量:", no_record_chars.length, "需要新增的資料筆數:", inserts.length);
...
async function regenerateAllStaticFont(state, have_gen_list, onlyFontIds = null) {
	...
	try {
		...
		for (const { ff_name, support_weights } of all_fonts) {
			currentIndex++;
			logger.debug(`正在確認字型 ${ff_name} ${support_weights} 的靜態生成 (${currentIndex}/${all_fonts_length})`);
			const this_font = {
				version: version_num,
			...
			logger.info(`╔ ${ff_name} ${support_weights} 有 ${charArray.length} 個字`);
			// 1. 查出資料紀錄不完全的字
			//       |資料庫存在該字|自型支援清單|
			...
			if (this_static_font_dir_status) {
				...
				logger.info(`╠ ${ff_name}-${support_weights} 應該共有 ${all_need_gen_pack.length} 包字型。本地只有其中 ${existPack.length} 包字體`);
			} else {
				ready_regen = all_pack_numbers;
			}
			logger.info(`╠ 生成 ${ff_name} ${support_weights} 缺少的 ${all_pack_numbers.length - existPack.length} 包靜態字型`);
			//重新生成
				...
				for (const res of settled) {
					if (res.status === "fulfilled") {
						if (res.value.success == false) {
							logger.error("生成靜態字型的多執行序發生錯誤！生成中斷！", res.value);
...
async function find_static_font(word_set, hash) {
	try {
        ...
        `; //不會比對該字型是否支援這個字，優點是不用根據不同的字體就重新去查，資料庫有紀錄的字都會給 pack number ，目標是通用兼容。如果請求到字型沒有的會給出不存在的包，前端會跳 404
			...
			return res.rows.map(row => String(row.pack).padStart(3, "0")).sort((a, b) => Number(a) - Number(b));
		} else {
