    ...
    try {
        return new Promise((resolve, reject) => {
            const worker = new Worker(
                path.resolve(__dirname, "font_nomin_worker.js"),
                {
                    workerData: data,
                }
            );
...
async function complete_ff_name_support_char_in_db(
    ff_name,
    charArray,
    existingChars
) {
...
async function regenerateAllStaticFont(state, have_gen_list) {
    ...
    try {
        ...
        for (const { ff_name, support_weights } of all_fonts) {
            ...
            //讀字型檔案，取出所有支援的字型
            const readFile_res = await readFontBuffer(
                ff_name,
                support_weights,
                true
            );
            ...
            const { rows: existing } = await db.query(
                "SELECT char FROM static_fonts WHERE char = ANY($1) AND  $2 =  any(families) ",
                [charArray, ff_name]
            );
            ...
            await complete_ff_name_support_char_in_db(
                ff_name,
                charArray,
                existingChars
            );
            ...
            console.log(
                `╠ 正在生成 ${ff_name} ${support_weights} 缺少的 ${
                    all_pack_numbers.length - existPack.length
                } 包靜態字型`
            );
            //重新生成
