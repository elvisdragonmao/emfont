...
const configuredDynamicFontCacheMaxSize = Number.parseInt(process.env.DYNAMIC_FONT_CACHE_MAX_SIZE || "5000", 10);
const DYNAMIC_FONT_CACHE_MAX_SIZE = Number.isFinite(configuredDynamicFontCacheMaxSize) && configuredDynamicFontCacheMaxSize > 0 ? configuredDynamicFontCacheMaxSize : 5000;
...
async function find_dynamic_font({ word_hash, font_id, font_family, font_weight, original_word_set, state }) {
	...
	if (cached) {
		...
		logger.debug(`dynamic font ${little_font_package} cache hit, returning cached URL: ${cached.url}`);
		return {
			status: "success",
			location: cached.url
		};
...
async function findOrGenerateDynamicFont({ little_font_package, font_family, font_weight, original_word_set, state }) {
	const localPath = path.join(__dirname, "..", "..", "_data", "_generated", little_font_package);
	...
	logger.debug(`Checking dynamic font: ${little_font_package}, cache exist: ${file_exist}`);
	if (file_exist) {
		setDynamicFontCache(little_font_package, file_url, DYNAMIC_FONT_EXISTING_FILE_TTL_MS);
		logger.debug(`動態字型檔 ${little_font_package} 已存在，直接回傳 URL 快取: ${file_url}`);
		return {
			status: "success",
			location: file_url
		};
	...
	try {
		const generated = await generateFont(font_family, font_weight, original_word_set, little_font_package);
		if (generated.status === "failed") {
			return generated;
		}
		const generatedUrl = `${state.baseURL}/_generated/${generated.location}`;
		setDynamicFontCache(little_font_package, generatedUrl, DYNAMIC_FONT_GENERATED_TTL_MS);
		return {
			status: "success",
